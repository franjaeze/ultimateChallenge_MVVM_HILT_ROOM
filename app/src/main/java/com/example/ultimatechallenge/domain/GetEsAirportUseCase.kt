...
class GetEsAirportsUseCase  @Inject constructor(private val repository: AirportRepository) {
    suspend operator fun invoke():List<Airport>{
        val dbAirports =  repository.getEsAirportsFromDatabase()
        if(!dbAirports.isNullOrEmpty()){
            Log.d("Base de datos","Estoy trayendo los Airports de la BD")
            return dbAirports
        }else {
            val airports = repository.getEsAirportFromApi()
