...
class GetAirportsUseCase @Inject constructor(private val repository: AirportRepository) {
    suspend operator fun invoke():List<Airport>{
        val dbAirports =  repository.getAllArgAirportsFromDatabase()
        if(!dbAirports.isNullOrEmpty()){
            Log.d("Base de datos","Estoy trayendo los Airports de la BD")
            return dbAirports
        }else {
            val airports = repository.getArgAirportFromApi()
